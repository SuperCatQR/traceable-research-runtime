...
- **架构 B：Web Search 版**。原文来自公网。强模型生成搜索词，从每个搜索词的候选网页中选择要打开的页面，Runtime 抓取并固化网页快照。免建重型文档库、覆盖广，但来源质量、可抓取性与稳定性依赖公网。
...
推论：抓取后端（trafilatura / 自托管 Firecrawl / Jina Reader 等）是 Source Module 内部可插拔实现，不影响三工具契约（`search_candidates / open_source / read_source`），可后期随意更换或并存。唯一硬约束——快照与 content_hash 的固化必须在自有信任边界内完成，不得把证据链托付给第三方托管抓取。
...
- `open(candidate_id)` 校验 URL 与访问策略后抓取网页，固化可重放快照，`source_ref` 形如 `source:web/snapshot-87#section-12`。
...
4. Runtime 校验候选 ID 与访问策略，`open` 所选原文并锁定版本/固化快照；失败者不进入证据流程。
...
> 所有专业问题沿同一固定数据流运行；强模型生成查询并从有界候选中选择原文，Runtime 校验、锁定版本或固化快照，廉价模型在授权原文内提取证据；原文来源可为结构化索引库（架构 A）或 Web Search（架构 B），二者共享同一 Source 契约与证据、审计协议，最终生成可控、可溯源、可审计答案。
