...
网页获取收敛为三个受控动作，实现在 `poc/search_mcp/server.py`。它们以 FastMCP 声明，也可由研究编排层进程内直调；PoC 走后者。
...
`research_session.py` 只持有 `snapshot.make_reader()` 返回的 reader；`server.py` 的 `open_source` 只持有 writer；`run.py` 两者都不持有。三者均不直接访问 SQLite：
...
sequenceDiagram
    ...
    loop 固定 N 轮，默认 N=3
        ...
        X->>A: 记 queries + candidates
        loop 全部去重候选，无模型预选
            X->>O: open_source(candidate_id)
            O->>O: 抓取前 SSRF 校验
            O->>C: 结构化抓取
            ...
            alt 抓取有效
                ...
                P-->>X: source_ref + content_hash
                X->>A: 记 opened
            else 抓取失败或正文无效
                O-->>X: open_skip + reason
                X->>A: 记 open_skip
            end
        end
    end
...
2. 研究编排层校验 source_ref 属本 run，随后从 snapshot reader 读取这些原文。
