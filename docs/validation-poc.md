...
对应时序。PoC 里 `run.py` 即 Runtime，编排环路，`server.py` 的三工具进程内 import（非 stdio）；模型（DeepSeek API）、crawl4ai（服务器 `192.168.1.71`）、搜索源是环路之外的独立服务，由 `server.py` 或环路按需调用：
...
sequenceDiagram
    ...
    participant SE as 搜索源<br/>ddgs
    participant CR as crawl4ai<br/>抓取服务·192.168.1.71
    ...
    loop 每个选中候选
        ...
        SRV->>CR: 抓取 + JS 渲染 + 抽正文
        CR-->>SRV: markdown 正文
        SRV->>SRV: 算 content_hash、生成 source_ref、写 snapshots/
