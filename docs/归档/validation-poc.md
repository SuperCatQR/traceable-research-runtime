...
open_source(candidate_id)        → {source_ref, source_uri, title, content_hash, char_len, fetched_at}  # 抓取并存档快照
...
| 术语 | 含义 |
| --- | --- |
| 架构 B（Web Search） | 用公网搜索 + 网页抓取取证的方案；对照架构 A 是结构化索引库。本 PoC 落地 B |
| Source 契约 | `search_candidates / open_source / read_source` 三个操作的统一接口，架构 A/B 共享，只有原文来源不同 |
| 候选（candidate） | 搜索返回的一条导航结果（标题 + 摘要 + URL），仅用于导航，本身不是证据 |
| `candidate_id` | 本 run 内候选的编号；模型只能从中选，不得自造 URL，`open_source` 只接受集合内的 id |
| 快照（snapshot） | `open_source` 抓取网页后存档的原文副本，冻结不改，存于 `snapshots/` |
| `source_ref` | 快照的稳定引用地址，形如 `source:web/<snapshot_id>`，引文靠它回指原文 |
| `content_hash` | 快照正文的哈希；存档时算一次，取证/读取时再校验，保证引用的原文未被篡改 |
| Evidence（证据） | 从某个快照中逐字摘出的、能支撑回答的原文片段，须通过 hash 与逐字命中校验 |
| Claim（主张） | 最终答案里的一条事实性陈述，每条都必须指向已验证的 Evidence |
| research-strong | 强模型角色：生成搜索词、选候选、产出 Claim 与带引用答案 |
| research-cheap | 廉价模型角色：只在授权快照内提取逐字证据 |
| 逐字命中 | 证据引文必须是快照正文中连续原样出现的片段（代码里的 `quote in text`），不得改写、拼接、翻译 |
| URL 守卫（`_is_public_http`） | 抓取前后各校验一次目标地址，拦内网地址、非 HTTP(S)、重定向越界，防 SSRF |
| trace | 每个 run 一份 `trace/<run_id>.jsonl`，逐步记录全过程，供事后回放与审计 |
| run | 一次完整问答的执行实例；状态与结论落 `store.sqlite` 的 run/evidence/claim 三表 |
| 金标（gold） | `eval/gold.jsonl` 里的评测题集：问题 + 参考答案 + 权威来源 |
...
对应时序。PoC 里 `run.py` 即 Runtime，编排环路，`server.py` 的三工具进程内 import（非 stdio）；模型（DeepSeek API）、crawl4ai（独立抓取服务）、搜索源是环路之外的独立服务，由 `server.py` 或环路按需调用。两个持久层职责不同：**快照存档 `snapshots/`** 由 `server.py` 独占读写，存网页原文供引文比对；**审计存储 `store.sqlite`（run/evidence/claim 三表）+ `trace/<run_id>.jsonl`** 由 `run.py` 独占写入，前者记结构化结论、后者逐步追加可回放的轨迹。为看清主干，下图画单次取证的线性流程（搜索词、候选实际会重复多次，此处不展开）：
...
sequenceDiagram
    ...
    participant SNAP as 快照存档<br/>snapshots/（server.py）
    participant AUD as 审计存储<br/>store.sqlite + trace/（run.py）

    U->>R: 问题
    R->>AUD: 记 question（trace）
    R->>S: 生成搜索词
    S-->>R: 搜索词（≤3）
    R->>R: 校验数量与 schema
    ...
    SE-->>SRV: 结果
    SRV-->>R: 有界结果列表（≤5，仅导航）
    R->>AUD: 记 candidates（trace）
    R->>S: 问题与结果列表
    S-->>R: 待打开 candidate_id（≤4）
    R->>R: 校验候选 ID 归属本 run
    R->>SRV: open_source(candidate_id)
    SRV->>SRV: URL 守卫（内网/协议）
    SRV->>CR: 抓取 + JS 渲染 + 抽正文
    CR-->>SRV: markdown 正文
    SRV->>SRV: 重定向落点复检、算 content_hash、生成 source_ref
    SRV->>SNAP: 写快照（source_ref, hash, 正文）
    SRV-->>R: source_ref + content_hash
    R->>AUD: 记 opened（trace）
    R->>SRV: read_source(source_ref)
    SRV->>SNAP: 按 source_ref 读快照
    SNAP-->>SRV: 存档正文 + hash
    SRV-->>R: 快照正文
    R->>C: 问题与快照正文
    C-->>R: 逐字证据候选
    R->>R: 校验 hash 匹配、引文逐字命中、去重
    R->>AUD: 记 evidence_check（trace）
    R->>S: 已验证证据菜单
    S-->>R: Claim + 带引用答案
    R->>R: 校验每条 Claim 指向已验证 Evidence
    R->>AUD: 写 run/evidence/claim（store.sqlite）+ 记 answer（trace）
    R->>U: 答案 + 引用
    Note over R,AUD: 任一环节无结果即中止不作答（无候选 / 存档全失败 / 无逐字命中证据），中止原因同样写入 trace
```
