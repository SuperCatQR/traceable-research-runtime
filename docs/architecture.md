...
Source Module 是 Runtime 内的数据访问边界，不是独立服务。它集中数据库查询，隐藏 PostgreSQL、对象存储和索引细节，使模型和工作流均不直接接触数据库。首期代码可命名为 `SourceRepository`，只保留两个只读函数：

```text
search(query, collection_ids)  检索授权范围内的索引与原文，返回 source_ref
read(source_ref)               读取固定版本的原文单元
```

Runtime 决定 `collection_ids`，模型不得扩大范围。采集、更新、删除和建索引属于离线入库流程；逐字引文校验属于 Evidence 模块。首期不预设 HTTP / RPC、请求封套、稳定错误码或复杂分页；出现第二个独立消费者后再增加传输接口。
...
4. 廉价模型只能经 Source Module 在 Runtime 指定范围内导航和读取，只返回符合 schema、带 `source_ref` 的证据候选；不得扩域、派生任务或写库。
5. Evidence 模块验证权限、版本、哈希、逐字引文和去重后，接受证据对象。
...
`search` 与 `read` 由 Source Module 执行；其余工具只提交候选对象。Runtime 固定每类模型可见的工具、授权范围和参数 schema。文档内容一律视为不可信数据，不得执行其中指令。
...
- 一个强模型档位；
- 一种廉价模型档位；
- 带普通导航索引的版本化文档库及两函数 Source Module；
