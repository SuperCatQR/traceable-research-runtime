...
- `schema_version`：仅 `run_header` 携带；当前为 **v2**。读取方仍须接受无 `run_failed` 的 v1 历史日志。
- `type`：v2 事件枚举为 `run_header | query | search_result | archive | archive_skip | excerpt | snapshot_selection | claim | answer | run_failed`。
...
7. 删除引文 substring 校验，改为 snapshot\_ref 归属、哈希和 Claim 引用范围校验。
