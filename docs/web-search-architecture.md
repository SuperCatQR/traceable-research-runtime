...
- `src/web.rs` 是 HTTP 薄入口；它转交原始问题、当前回答、确认与取消，并将错误映射为状态码；客户端不能编辑 Brief。
...
sequenceDiagram
    ...
    U->>W: POST /api/research/intakes {question}
    W->>I: create(question)
    ...
    loop 回答当前质询，累计最多展示 5 个且用户尚未确认或取消
        U->>W: POST /api/research/intakes/{id}/reply {revision, answer}
    ...
    alt 用户确认任一版当前草案
        U->>W: POST /api/research/intakes/{id}/confirm {revision, content_hash}
        ...
        U->>W: POST /api/research/intakes/{id}/cancel {revision}
        W->>I: cancel
...
HTTP 边界只需四个写端点。客户端不能在创建或回复请求中提交 `brief_draft`；草案仅由服务根据原问题和完整问答历史生成：

```text
POST /api/research/intakes
  {question} -> {clarification_id, revision, status, brief_draft, question, questions_asked}

POST /api/research/intakes/{clarification_id}/reply
  {revision, answer} -> {revision, status, brief_draft, question, questions_asked}
...
1. **已有 Intake 基础**：`src/intake.rs` 已落地 Brief schema、澄清状态机、幂等确认及 `data/intake/<clarification_id>.jsonl` 回放；创建入口已只接收 `question`，服务会生成草案；`src/web.rs` 已暴露创建、回复、确认、取消四组 API，Web UI 全程走 Intake。
