...
Source Interface 是模型及证据校验模块读取数据源的唯一边界；首期作为 Runtime 进程内、文档库前的独立模块，不另部署服务。模型提出工具请求，Runtime 代为调用并返回结果，故 Model API Gateway 无须跨网直连 Source 或持数据库凭据。它隐藏 PostgreSQL、对象存储和索引实现，仅暴露四个只读操作：
