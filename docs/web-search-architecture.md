...
| Search Provider Attempt | 对某个查询、Brave Search API 的一次显式请求及 typed 结果 | 隐式重试或聚合搜索 |
| Web Snapshot | 从公开网页安全抓取、经进程内提取后保存的内容寻址正文 | 搜索结果 title/snippet |
...
flowchart LR
    ...
    subgraph Core["traceable-search library"]
        ...
        search["web_search.rs\nWebSearch seam"]
        websnapshot["web_snapshot.rs\n安全抓取与进程内提取"]
        brave["brave_search.rs\nBrave Search API"]
        model_adapter["model_adapter.rs\nOpenAI-compatible HTTP"]
...
| 模块 | 主要接口 | 责任 | 明确不负责 |
| --- | --- | --- | --- |
| `src/lib.rs` | crate root 重导出 | 声明 library-only crate，公开实现模块，并把主要领域类型、Runtime、Trace 和 Snapshot 接口重导出为平坦调用面 | HTTP、身份、进程启动 |
...
| `src/brave_search.rs` | `BraveSearchClient` | 实现 Brave Search API HTTP、响应校验和结果投影 | Run 状态、Trace 顺序、网页正文提取 |
| `src/model_adapter.rs` | `OpenAiCompatibleModelClient` | 实现 OpenAI-compatible JSON 请求和模型端点安全策略 | Run 状态、搜索、网页正文提取 |
| `src/web_search.rs` | `WebSearch` | 定义单方法搜索接缝；生产使用 Brave Adapter，测试使用固定结果 Adapter | 搜索 HTTP 细节、Trace 写入 |
| `src/web_snapshot.rs` | `EmbeddedSnapshotClient`、`validate_public_web_url` | 执行公网 URL/DNS/重定向校验、限量抓取、HTML allowlist、标题提取、进程内 Markdown 转换和 Snapshot 构造 | 搜索、选源、JavaScript 渲染 |
| `src/live_research_backend.rs` | `LiveResearchBackend` 与五类 prompt 常量 | 把 `WebSearch`、进程内 Snapshot 和模型 Adapter 组合为 `ResearchExecutionBackend`；定义每类模型调用可见的数据形状和“不可信数据”提示契约 | 校验模型 JSON、持久化、资源计数 |
...
| `tests/live_e2e.rs` | 默认 ignored 的真实 Brave/public Web/model 快乐路径测试；当前不能作为日常构建门禁 |
...
| 分组 | 命令 | 契约 |
| --- | --- | --- |
| 配置 | `ResearchInfrastructureConfig::from_env` | 读取 Brave API key 和 Core 数据目录；进程级共享 |
...
2. 对每个查询执行一次 Brave Search API 请求；每个查询必须产生 typed completed 或 failed。正常空结果也是 completed。
3. 记录 Search Attempt 和 Search Result；任一查询的 search completion 为 failed 会终止整个 Run。
...
```mermaid
stateDiagram-v2
    [*] --> Brave: HTTPS API request
    Brave --> Completed: completed (含可证明空结果)
    Brave --> Failed: unavailable 或 contract_rejected
```

`BraveSearchClient` 对每个 query 发送一次请求：

1. 使用服务端 `BRAVE_SEARCH_API_KEY`，通过 `X-Subscription-Token` 请求官方 Web Search endpoint。
2. 查询同时发送 `count=10`、`search_lang` 和 `ui_lang`；中文查询使用 `zh-hans`/`zh-CN`，其他查询使用 `en`/`en-US`。
3. 只接受 `web.results[]` 中的 HTTP(S) URL，最多保留 10 条并重新生成 `search_result_id`。
4. 429、408、5xx 和网络超时记录为 typed unavailable；其他非 2xx、无效 JSON 或缺少 `web` 结构记录为 contract rejected。
5. 搜索结果只负责导航；每个 URL 仍必须经过后续网页 DNS/SSRF 校验和 Snapshot 流程。

应用不直连 Google/Bing 搜索页，也不部署 SearXNG。Trace 每个 query 记录一个 `brave` attempt 和结果 count/rank；旧 v7 Trace 中的 Google/Bing attempt/fallback 仍可回放。

## 9. 公网页面抓取、进程内提取与 Snapshot

### 9.1 公网页面获取

安全获取实现位于 `web_snapshot.rs`，由 `EmbeddedSnapshotClient` 通过唯一公开操作 `capture_web_snapshot(url)` 调用。
...
- HTML 经 allowlist 清洗，移除属性、脚本、媒体和 URL，再由 `htmd` 在进程内转换为 Markdown。

当前获取器不检查页面响应是否为 2xx，也不校验 `Content-Type`。它没有确定性识别登录墙、付费墙、CAPTCHA 或反爬页面；这些页面只有在网络失败或最终 Markdown 为空时才会成为 `archive_skip`。

### 9.2 Embedded Snapshot 提取

`EmbeddedSnapshotClient` 从原 HTML 提取规范化 title，将安全 HTML 转换为 Markdown，拒绝空正文，并记录 extractor/version metadata。归档正文上限是 4 MiB，超限时在 UTF-8 边界截断并记录 `truncated=true`。它不执行 JavaScript，也不加载被清洗掉的子资源。

Snapshot 的 `crawl`、`CrawlMeta` 和 `CrawlBodyKind` 名称为兼容旧 SQLite/JSON 数据而保留；新实现只写 `RawMarkdown`，`fit_markdown_bytes` 为 0，metadata 标明 embedded extractor。旧 Snapshot 不会被重写。
...
- Search Query、Brave attempt、result 与 `RoundCompleted` 顺序一致；旧 Google/Bing fallback 顺序仍可回放；
...
Conversation 历史、模型输出、搜索 metadata、网页正文和提取 metadata 都作为数据处理。Prompt 要求模型忽略嵌入指令；程序另外通过严格 JSON、引用集合、URL、hash 和状态机限制可观察控制面。Prompt injection 的语义抵抗仍依赖模型，不能由 schema 校验完全证明。
...
- 搜索和模型 adapter 没有无限重试；Brave 的有限网络重试由 API/部署层控制，并必须记录 attempt。
...
- `/health` 只证明 Host listener 存活，不证明 Brave API、模型、公网或持久卷可用。
...
| 配置 | 所有者 | 作用 |
| --- | --- | --- |
| `BRAVE_SEARCH_API_KEY` | Core process | Brave Search API 服务端密钥 |
...
- Containerfile 的 final image 同源提供 Browser 和 `/api`，并包含用于容器健康检查的 curl。
- 根目录 `compose.yaml` 声明 App、Brave API key、健康检查和研究数据卷；不部署搜索引擎或抓取服务。
- 仓库不规定服务器替换或回滚流程；部署环境必须让 Host 能访问 Brave API、公开网页和用户 Model Profile 端点。
- 替换实例前应使用 Brave API 探针确认认证、额度和至少一条有效 HTTP(S) 结果；该门禁由部署环境负责。
...
当前 `tests/live_e2e.rs` 断言所有公开 claim 都有 sources，但 Core 契约要求至少一条 `model_knowledge` claim 无 Web source，因此该 ignored 测试与当前答案契约不一致。它也不验证 Brave API 额度/响应和 Trace v7 replay，不能作为这些设计的证明。

本次改造已本地运行 Core 113 项测试、Host 15 项测试、Browser 回归测试、Core/Host Clippy、Core/Host release build、TypeScript check 和 Vite production build，均通过。Compose YAML 已解析和格式校验，但当前 Windows/WSL 环境没有 Docker/Podman Compose，因此未启动容器、未执行真实搜索探针、真实外部依赖 E2E 或 Browser 截图。
...
- 搜索服务商比较：[2026-07-20-search-api-provider-comparison.md](decisions/2026-07-20-search-api-provider-comparison.md)。
- [ADR 0009](adr/0009-require-complete-v7-trace-replay-before-projection.md)：投影和恢复前必须完整回放 Trace v7。
- [ADR 0010](adr/0010-embed-snapshot-extraction-and-keep-search-at-a-seam.md)：内收 Snapshot 提取并把搜索保留在稳定接缝。
